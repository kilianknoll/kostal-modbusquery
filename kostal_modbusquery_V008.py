...
#   Added Function WriteS16 function to be able to write certain registers
#   Register 1034 : Battery charge power (DC) setpoint, absolute
#   Register 1038 : Battery max. charge power limit, absolute
#   Register 1040 Battery max. discharge power limit, absolute
#   Register 1042 Minimum SOC
#   Register 1044 Maximum SOC
...
class kostal_modbusquery:
    ...
    #-----------------------------------------
    # Routine to write registers S16
    def WriteS16(self,myadr_dec,value):
        ...
        except Exception as BadWrite:
            print ("Error in function WriteS16 writing Register ", myadr_dec, "  ", BadWrite)
...
if __name__ == "__main__":  
    ...
    print ("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX") 
    print ("When you read these lines Sourcecode below shows how to set the Battery charging / Discharging values")
    print ("This only works when you:")
    print ("    -Have a battery connected to the inverter")
    print ("    -Have external Battery Control via Modbus enabled")
    print ("If all of this is satisfied, you may tweak the source code accordingly")
    print ("Please set the variable Write_enabled to 1")
    Write_enabled = 0
    Batterycharge = 100 # This is the Power [Watt] we charge (positive Value) or discharge (negative Value)
    if Write_enabled ==1:
        print ("Starting to try and write Registers...")
        print ("XXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXXX")
        MyValue =-100
        if MyValue <0:
            print("Value is set to charge the battery with :", abs(MyValue), "[Watt]")
        else:
            print("Value is set to discharge the battery with :", abs(MyValue), "[Watt]")
        MyWriteregister = 1034 # Register 1034 : Battery charge power (DC) setpoint, absolute
        Kostalquery.WriteS16(MyWriteregister,MyValue)
        print ("exiting write, Look at your inverter´s webpage in parallel ;-) ")
